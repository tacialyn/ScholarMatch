...
// ---- Defensive helpers ----
// Handles the fact that some scholarship entries store benefits/requirements
// as arrays, and others as plain strings.
function toList(val) {
...
// Handles the fact that some entries nest "source" inside "contact",
// while others (the DOST-SEI entries) put it as a sibling of "contact".
function getSource(scholarship) {
  return (scholarship.contact && scholarship.contact.source) || scholarship.source || null;
}

// Small helper so missing fields render "N/A" instead of "undefined".
function safe(val) {
